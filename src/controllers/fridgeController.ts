...
const getFridgeList = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const list = await fridgeService.getFridgeList(userId);
...
const createFridge = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user!.id;
...
const updateFridge = async (req: AuthRequest<{ id: string }>, res: Response) => {
    try {
        const userId = req.user!.id;
        ...
        const updatedFridge = await fridgeService.updateFridge(userId, id, name);
        ...
        if (error instanceof Error) {
            if (error.message === "NOT_FOUND_FRIDGE") {
                res.status(404).json({
                    message: "존재하지 않거나 이미 삭제된 냉장고입니다.",
                });
                return;
            }
...
const deleteFridge = async (req: AuthRequest<{ id: string }>, res: Response) => {
    try {
        const userId = req.user!.id;
        const id = Number(req.params.id);
        ...
        await fridgeService.deleteFridge(userId, id);
